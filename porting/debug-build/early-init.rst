...
There are several cases in which telnet will not be exposed, such as when the device fails to load the kernel or initramfs, or when an in-kernel driver decides to cause a kernel panic very early. In this case, the phone will almost immediately reboot when it starts, even when using ``hybris-recovery``. To see why this is happening, you may be able to :doc:`read the previous boot's kernel message buffer <dmesg>`. Please have that ready if you :ref:`contact us for help <support-channels>`.
